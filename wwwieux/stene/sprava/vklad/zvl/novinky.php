...
if(isset($jak)){
	if($jak == "vypis"){
		...
		while($novina = $novinky->fetch()){
			echo "<tr>
			<td><a href=\"?jak=upravit&upravit=" . $co . "&kuprave=".$novina["cis"]."\" title=\"upravit údaje\">".$novina["nazev"]."</a></td>
			<td><a href=\"?jak=upravit&upravit=" . $co . "&kuprave=".$novina["cis"]."\" title=\"upravit údaje\">".datum(strtotime($novina["kdy"]))."</a></td>
			<td class=\"smazat\"><a href=\"akce/zapsat.php?upravit=" . $co . "&kuprave=" . $novina["cis"] . "&jak=zobr&pryc=\" title=\"odstranit\">smazat</a></td>
			</tr>\n";
		}
		echo "</table>";
	}elseif($jak == "upravit"){
		...
		if(!empty($novina)){
			echo " <em>".$novina["nazev"]." (".datum(strtotime($novina["datum"]));
		}
		echo ")</em></legend>\n";
		if(isset($_GET["nova"])){
...
//		echo "<a href=\"?jak=upravit&upravit=adresare&kuprave=".$novina["cis"]."&sekce=novinky\" title=\"Vytvořit adresář\">Vytvořit adresáře pro fotky</a><br>\n";
		echo "<a href=\"akce/foto-novinky.php\" title=\"Vytvořit adresář\">Vytvořit adresáře pro fotky</a><br>\n";
		echo "<a href=\"akce/nacist-fotky.php?jak=upravit&upravit=fotky&kuprave=".$novina["cis"]."&sekce=" . $_GET["upravit"] . "\" title=\"Načíst fotky\">Načíst fotky</a><br>\n";
...
//putInput($zDtB, $nazvy, $co, $klic, $size = 60, $typ = false, $cols=100, $rows=35)	
		...
		echo "></label>\n";
		echo "<label for=\"na-struk\">Struknews<input type=\"checkbox\" id=\"na-struk\" name=\"struk\" value=\"1\"";
		if(!is_null($novina["struk"])){
			echo " checked";
		}
		echo "></label>\n";
